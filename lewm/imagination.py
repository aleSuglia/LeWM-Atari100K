...
class ImaginationEnv:
    ...
    def bind_world_model(self, world_model):
        """Rebind model/device after external wrapping or device moves."""
        self.world_model = world_model
        self.device = next(world_model.parameters()).device
    ...
    def reset(self, batch_size=None):
        if batch_size is None:
            raise ValueError(
                "batch_size must be provided when resetting imagination env"
            )
    ...
    def step(self, action):
        if (
            self.elapsed is None
            or self.emb_history is None
            or self.emb_act_history is None
        ):
            raise RuntimeError("ImaginationEnv.step() called before reset().")

        emb_history = self.emb_history
        emb_act_history = self.emb_act_history
        elapsed = self.elapsed

        act_emb = self.world_model.encode_action(action).unsqueeze(1)
        emb_act_history = torch.cat([emb_act_history, act_emb], dim=1)

        next_emb, reward, done_logit = self.world_model.transition(
            emb_history, emb_act_history
        )
        ...
        elapsed = elapsed + 1
        truncated = elapsed >= self.max_horizon

        done = terminated | truncated

        self.elapsed = elapsed
        self.emb_history = torch.cat([emb_history, next_emb.unsqueeze(1)], dim=1)[
            :, -self.history_size :
        ]
        self.emb_act_history = emb_act_history[:, -self.history_size + 1 :]
